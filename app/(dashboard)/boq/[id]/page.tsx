...
export default function BOQPage({ params }: { params: { id: string } }) {
  ...
  const handleSave = async (customerName: string, customerPhone: string, customerAddress: string) => {
    ...
    try {
      ...
      console.log("🔵 Starting BOQ save with user ID:", profileId);
      ...
      if (!currentLeadId) {
        ...
        console.log("✅ Lead created:", currentLeadId);
      }
...
if (boqId) {
      ...
      console.log("✅ BOQ saved:", boqId);

      console.log("📝 Inserting BOQ items...", activeItems.length);
    ...
    if (itemsError) {
      ...
      console.log("✅ All BOQ items saved successfully");
      ...
      await fetchAllBOQs();
      console.log("🎉 BOQ save complete and UI refreshed");
            ...
            <table className="w-full text-left text-sm border-collapse">
              ...
              <tbody className="divide-y">
                {loadingBOQs ? (
                  ...
                  <tr>
                    <td colSpan={7} className="p-8 text-center text-red-500 font-bold bg-red-50">Error: {fetchError}</td>
                  </tr>
                ) : allBOQs.length === 0 ? (
                  <tr>
