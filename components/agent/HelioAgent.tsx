...
// ── Chat Panel ────────────────────────────────────────────────────────────────
function ChatPanel({ messages, input, setInput, onSend, sending, onClose, isRtl, loginStep, resolvedEmail, onLoginSuccess }: {
  messages: Msg[]; input: string; setInput: (v: string) => void;
  onSend: () => void; sending: boolean; onClose: () => void; isRtl: boolean;
  loginStep?: 'name' | 'password' | 'done';
    ...
    >
      ...
      <div style={{
        ...
        <div style={{ flex: 1, direction: 'rtl' }}>
          ...
          <div style={{ color: 'rgba(255,255,255,0.65)', fontSize: 11 }}>HELIOMAX</div>
        </div>
      ...
      {/* T059: PasswordStep inline during conversational login */}
      {loginStep === 'password' && resolvedEmail && onLoginSuccess && (
        <div style={{ padding: '0 10px 10px', background: '#0a1929' }}>
          <PasswordStep email={resolvedEmail} onSuccess={onLoginSuccess} />
        </div>
      )}
...
// ── Main Component ────────────────────────────────────────────────────────────
export default function HelioAgent() {
  ...
  // T059: conversational login state
  const [loginStep, setLoginStep] = useState<'name' | 'password' | 'done'>('name');
  const [resolvedEmail, setResolvedEmail] = useState<string | null>(null);

  const hasGreetedRef = useRef(false);
  const hasLoginGreetedRef = useRef(false);
  ...
  // Auto-open with greeting on /login page for logged-out users
  useEffect(() => {
    if (pathname !== '/login' || authLoading || profile || hasLoginGreetedRef.current) return;
    hasLoginGreetedRef.current = true;
    const t = setTimeout(() => {
      setMessages([{
        role: 'assistant',
        content: 'أهلاً! أنا هيليو 👋\nاكتب اسمك الأول عشان أعرفك وأفتحلك الداشبورد.',
      }]);
      setChatOpen(true);
    }, 1200);
...
اسم المستخدم: ${profile.name} | دوره: ${profile.role} | الصفحة: ${page}.`;
  ...
  const handleSend = useCallback(async () => {
    ...
    // T059: when no profile, first user message = their name → look up email
    if (!profile && loginStep === 'name' && !resolvedEmail) {
      ...
      if (error || !data?.email) {
        setMessages(prev => [
          ...
          { role: 'assistant', content: `معرفتش ألاقي حساب بالاسم "${name}". ممكن تكتب الاسم تاني؟` },
        ]);
        return;
      }
      ...
      setMessages(prev => [
        ...prev,
        { role: 'user', content: name },
        { role: 'assistant', content: `أهلاً ${name}! دخّل كلمة مرورك في الخانة اللي تحت (Enter your password below).` },
      ]);
      return;
    }
...
الصفحة الحالية: ${pathname}. اسم المستخدم: ${profile?.name ?? 'زميل'}.`;
  ...
  const handleLoginSuccess = useCallback(async () => {
    ...
    setMessages(prev => [
      ...prev,
      { role: 'assistant', content: 'تم تسجيل الدخول بنجاح! جاري تحميل بياناتك...' },
    ]);
